...
from pic_plots_picPlots import *
from pic_plots_comboPlot import *
...
class multiPlotsSDT(multiPlots):
    ...
    def getCrops(self, name:str, kwargs2:dict, kwargs:dict) -> None:
        ...
        if 'crops' in kwargs:
            ...
            crops = {'relative':True}
            if name[:-1]=='HIx':
                crops = {**crops, 'w':200, 'h':150, 'wc':50, 'hc':40}
            elif name[:-1]=='HOx':
                crops = {**crops, 'w':100, 'h':250, 'wc':50, 'hc':50}
            elif name[:-1]=='HOh':
                crops = {**crops, 'w':1000, 'h':275, 'wc':500, 'hc':75}
            elif name[:-1]=='V':
                crops = {**crops, 'w':200, 'h':1000, 'wc':60, 'hc':500}
            kwargs2['crops'] = crops
