...
def testCSV(fn:str) -> str:
    '''get the path of the CSV for this unit test'''
    cdir = os.path.dirname(os.path.abspath(os.path.join('..')))
    return os.path.join(cdir, 'tests', f'test_{fn}.csv')  # the csv file for the test
...
class unitTester:
    ...
    def runTest(self, i:int, diag:int=0, **kwargs) -> Tuple[pd.Series, dict, list]:
        ...
        d,u = self.func(file, diag=diag, **kwargs).values()
