...
class qualityScatter(multiPlot):
    ...
    def __init__(self, ms:summaryMetric, ss:pd.DataFrame, xvar:str, yvar:str, simplify:bool=False, **kwargs):
        self.xvar = xvar
        self.yvar = yvar
...
class qualityScatterSpacing(multiPlot):
    ...
    def __init__(self, ms:summaryMetric, ss:pd.DataFrame, xvar:str, yvar:str, cvar0:str, **kwargs):
        self.xvar = xvar
        self.yvar = yvar
    ...
    def getRC(self) -> None:
        ...
        self.rows = 2
        self.cols = 5
        self.cvars = np.array([[self.cvar0 for i in range(4)], [self.relaxVar for i in range(4)]])
    ...
    def combineLegendBox(self) -> None:
        ...
        uniqueVals = pd.unique(self.ss[[self.cvar0, self.relaxVar]].values.ravel('K'))  # get the unique values in all columns together
        sslegen = pd.DataFrame([{'change':c} for c in uniqueVals])  # create a dataframe with all the possible morphologies
        sslegen.dropna(inplace=True)
        self.legendObj = scatterPlot(self.ms, sslegen, justLegend=True, ax=self.axbig, cvar='change', yvar='change', xvar='change', legendVals=list(sslegen['change']), fs=self.fs)
    ...
    def groupCols(self) -> None:
        ...
        self.axs[0,0].get_shared_y_axes().join(self.axs[1,0], self.axs[0,0])  # share the y axes for these columns
        self.axs[0,0].get_shared_x_axes().join(self.axs[1,0], self.axs[0,0])  # share the x axes for these columns
    ...
    def plot(self, i:int, j:int) -> None:
        ...
        self.objs[i,j] = scatterPlot(self.ms, ss
                                 , set_xlabel=(i==self.rows-1), set_ylabel=(j<2)
                                 , legend=False
                                 , legendloc='right'   
                                 , **kwargs)
...
class qualityScatterSimple(qualityScatterSpacing):
    ...
    def getRC(self) -> None:
        ...
        self.rows = 2
        self.cols = 2
        self.cvars = np.array([[self.cvar0], [self.relaxVar]])
