...
class regressionTable:
    ...
    def createVariableTable(self, scvar:str) -> pd.DataFrame:
        ...
        for prefix in ['ink_', 'sup_']:
            ...
            reg = self.regRow(xcol)
            reg['title'] = self.ms.varSymbol(f'{prefix}{scvar}', commas=False)
            df.append(reg)
        ...
        for suffix in ['Prod', 'Ratio']:
            ...
            reg = self.regRow(xcol)
            reg['title'] = self.ms.varSymbol(f'{scvar}{suffix}', commas=False)
            df.append(reg)       
        df = pd.DataFrame(df)
        return df
...
class regressionTables:
    ...
    def __init__(self, ms:summaryMetric, yvar:str, **kwargs):
        self.ms = ms
        self.yvar = yvar
        self.kwargs = kwargs
        self.run()
