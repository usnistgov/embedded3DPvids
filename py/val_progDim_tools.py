...
#         print(f'export {s}')
    ...
    def progPosRow(self, lineName:str) -> dict:
        ...
        if not pd.isna(timeRow.iloc[0]['lprog']):
            posRows = self.progPos[self.progPos.dprog==float(timeRow['lprog'])]
            if len(posRows)==0:
                print(self.progPos)
                print(timeRow)
                raise ValueError('Cannot find line in table')
        else:
            posRows = self.progPos
        posRowi = (posRows['t0']-time).abs().argmin()
        return posRows.iloc[posRowi]
    ...
    def lineMidPoint(self, lineName:str) -> dict:
    ...
    def relativeCoords(self, tag:str, writeLine:int=1) -> dict:
        ...
        lnum = tag[:2]   # l1, l2, etc.    
        # find the name of the first write line, e.g. l2w
        if writeLine==1:
            if f'{lnum}w' in list(self.progDims.name):
                write = f'{lnum}w'
            elif f'{lnum}w1' in list(self.progDims.name):
                write = f'{lnum}w1'
            else:
                raise ValueError(f'Could not find first write line for {tag}')
        else:
            if f'{lnum}w{writeLine}' in list(self.progDims.name):
                write = f'{lnum}w{writeLine}'
            else:
                raise ValueError(f'Could not find write line {writeLine} for {tag}')
        writePoint = self.lineMidPoint(write)
        ...
        if 'o' in tag:
            ...
            if tag==write:
                return d
            centerPointNow = self.lineMidPoint(tag)    # where this line is
#             print(writePoint, centerPointNow)
            # shift to get to the written line from the observe origin
