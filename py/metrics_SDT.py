...
def measureFolders(topFolder:str, overwrite:bool=False, horiz:bool=True, xs:bool=True, vert:bool=True, **kwargs) -> List[str]:
    '''measure all stills in folders recursively from the top folder'''
    errorFolders = []
    if not fh.isPrintFolder(topFolder):
        for f in os.listdir(topFolder):
            errorFolders = errorFolders + measureFolders(os.path.join(topFolder, f), overwrite=overwrite, horiz=horiz,xs=xs,vert=vert, **kwargs)
        return errorFolders
...
class metricDisturbSummary(metricSummary):
