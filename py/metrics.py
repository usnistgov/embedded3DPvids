...
def xsDisturbMeasure(file:str, acrit:int=100, diag:int=0, **kwargs) -> Tuple[dict,dict]:
    ...
    # segment components
    hc = 150
    crop = {'y0':hc, 'yf':h-hc, 'x0':200, 'xf':300}
...
def xsDisturbMeasures(folder:str, overwrite:bool=False, **kwargs) -> None:
    ...
    for f in os.listdir(folder):
        if 'still' in f:
            files[re.split('_', re.split('still_', f)[1])[1]] = f
...
def vertSegmentMeasure(df2:pd.DataFrame, markers:Tuple, im:np.array, im2:np.array, s:float, name:str, file:str, maxlen:float=20000, diag:int=0) -> Tuple[dict,dict]:
    ...
    units = {'line':'', 'aspect':'h/w', 'area':'px'
             ...
             , 'w':'px', 'h':'px'
             , 'xc':'px', 'yc':'px'
             , 'segments':'', 'vest':'px^3'} # where pixels are in original scale
...
def vertDisturbMeasure(file:str, acrit:int=2500, diag:int=0, **kwargs) -> Tuple[dict,dict]:
    ...
    # segment components
    hc = 0
    crop = {'y0':hc, 'yf':h-hc, 'x0':200, 'xf':330}
    im = vc.imcrop(im, crop)
    
    im2, markers, attempt = vm.segmentInterfaces(im, acrit=acrit, diag=max(0,diag-1), cutoffTop=0, botthresh=100, topthresh=100)
    if len(markers)==0 or markers[0]==1:
        return errorRet # nothing to measure here
    df = vm.markers2df(markers)
    df = df[(df.a>acrit)]   # remove small segments
    df = df[df.w<crop['xf']-crop['x0']]  # remove segments that are the width of the whole image
    retval, units = vertSegmentMeasure(df, markers, im, im2, s, name, file, maxlen=h, diag=diag)
    for s in ['x0', 'xc']:
        retval[s] = retval[s]+crop['x0']
    for s in ['y0', 'yc']:
        retval[s] = retval[s] + hc
